...
function buildStatValueLayout(template) {
  return {
    ...
    default_color: String(template?.layout?.stat_values?.default_color || 'white').trim() || 'white',
    winner_color: String(template?.layout?.stat_values?.winner_color || '0x32D74B').trim() || '0x32D74B',
  };
}
