...
function normalizeQueueStatusPresentation(presentation = {}) {
  return {
    ...DEFAULT_QUEUE_STATUS_PRESENTATION,
...
function formatScheduledForLabel(value, timeZone = 'UTC', notScheduledLabel = DEFAULT_QUEUE_STATUS_PRESENTATION.not_scheduled_label) {
  const iso = String(value || '').trim();
  if (!iso) {
    return String(notScheduledLabel || DEFAULT_QUEUE_STATUS_PRESENTATION.not_scheduled_label).trim();
  }
...
export function buildPokeQuizzQueueStatusPayload({
  ...
  const nextScheduledLabel = formatScheduledForLabel(
    ...
    effectivePresentation.not_scheduled_label,
  );
