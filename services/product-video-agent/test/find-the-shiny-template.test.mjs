...
test('visual filter starts with pokeballs, then reveals the grid with exactly one shiny cell and sparkle', async () => {
  ...
  assert.match(visualFilter.script, /\[timerhpbarbase\]scale=w='max\(2,iw\*\(if\(lt\(\(t\),2\.6\),0,/u);
  assert.match(visualFilter.script, /:h='max\(2,ih\*\(if\(lt\(\(t\),2\.6\),0,/u);
  assert.match(visualFilter.script, /:eval=frame,format=rgba,setsar=1\[timerhpbar\]/u);
