...
test('planner prefers scenic non-pokeball backgrounds when alternatives exist', async () => {
  const plan = await planPokemonTypeChallenge({
    template,
    pokedexRows,
    seed: 'prefer-scenic-background',
    forcedTypePair: ['grass', 'poison'],
    assetInventory: {
      scanned_at: '2026-07-31T00:00:00.000Z',
      directories: {},
      backgrounds: [
        '/tmp/Pokeballs Red Small.png',
        '/tmp/Forest.png',
        '/tmp/Pastel Blue Plain.png',
      ],
      music: ['/tmp/battle-intro-1.mp3'],
      sound_effects: {
        all: ['/tmp/countdown-tick.wav', '/tmp/reveal.wav'],
  ...
  assert.doesNotMatch(plan.assets.background.selected_path || '', /pokeball/i);
});
