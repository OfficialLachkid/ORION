...
test('render plan creates staggered round timing with slide transitions', async () => {
  ...
  assert.equal(dualTypeRound.type_badge_layout[1].center_x, 666);
...
test('visual filter composes round scenes and chains them with slideleft xfade transitions', async () => {
  ...
  assert.match(visualFilter.script, /setpts=PTS-STARTPTS,scale=896:896:force_original_aspect_ratio=decrease/u);
  const dualTypeRound = renderPlan.rounds.find((round) => round.subject.types.length === 2);
  assert.ok(dualTypeRound);
  assert.doesNotMatch(
    visualFilter.script,
    new RegExp(escapeRegExp(dualTypeRound.type_label), 'u'),
  );
  dualTypeRound.subject.types.forEach((type) => {
